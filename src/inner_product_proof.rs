...
impl InnerProductProof {
    ...
    pub fn from_bytes(slice: &[u8]) -> Result<InnerProductProof, ProofError> {
        ...
        let pos = 2 * lg_n * 32;
        let a: Option<Scalar> = Scalar::from_canonical_bytes(read32(&slice[pos..])).into();
        let a = a.ok_or(ProofError::FormatError)?;
        let b: Option<Scalar> = Scalar::from_canonical_bytes(read32(&slice[pos + 32..])).into();
        let b = b.ok_or(ProofError::FormatError)?;
