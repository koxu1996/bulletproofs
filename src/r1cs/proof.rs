...
impl R1CSProof {
    ...
    pub fn from_bytes(slice: &[u8]) -> Result<R1CSProof, R1CSError> {
        ...
        let t_x: Option<Scalar> = Scalar::from_canonical_bytes(read32!()).into();
        let t_x = t_x.ok_or(R1CSError::FormatError)?;
        let t_x_blinding: Option<Scalar> = Scalar::from_canonical_bytes(read32!()).into();
        let t_x_blinding = t_x_blinding.ok_or(R1CSError::FormatError)?;
        let e_blinding: Option<Scalar> = Scalar::from_canonical_bytes(read32!()).into();
        let e_blinding = e_blinding.ok_or(R1CSError::FormatError)?;
